...
namespace DayFour
{
    public class Parser
    {
        public IEnumerable<TokenCollection> Parse(string[] inputs, ValidationType validate = ValidationType.ValidatePassport)
        {
        ...
        private readonly Regex _hairColorRegex = new Regex("#([0-9|a-z]{6})");
        ...
        private readonly Regex _heightRegex = new Regex("(cm|in)", RegexOptions.Compiled);
        private bool ValidateHeightToken(Token token)
        {
            ...
            if (match.Success)
            {
                ...
                if (!int.TryParse(value[..index], out var number)) return false;
